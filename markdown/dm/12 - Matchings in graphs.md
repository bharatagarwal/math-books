...
![Figure 30: A bipartite graph and a perfect matching in it](12 - Matchings in graphs_images/img-2.jpeg)
...
Before we prove anything, let's see the claim hold on the actual object from the prom. We build a concrete $50$-regular bipartite graph on $150 + 150$ nodes with **networkx**, reproduce the book's edge count $150 \cdot 50 = 7500$, then run a real maximum-matching algorithm (Hopcroft-Karp) and check that the matching covers all $300$ nodes:

```python
<!-- include: code/dm/12 - Matchings in graphs/01_python.py -->
```

Running it prints `number of edges: 7500`, `students matched: 300 / 300`, and `dancing pairs: 150`, confirming Theorem 12.1: the $50$-regular bipartite graph on $300$ nodes has a perfect matching.
...
The tribes decide that they all should choose new totems. They decide that each tribe should pick one of the six species of tortoise that live on the island. Of course, they want to pick different totems, and so that the totem of each tribe should occur somewhere on their territory.

It is given that the areas where the different species of tortoises live don't overlap, and they have the same area - 100 square miles (so it also follows that some tortoise lives everywhere on the island). Of course, the way the tortoises divide up the islands may be entirely different from the way the tribes do (Figure 31).
...
![Figure 32: The graph of tribes and tortoises](12 - Matchings in graphs_images/img-4.jpeg)
...
We can check this directly on the graph of Figure 32. Encoding the bipartite graph in **networkx** (tribe $A$ has $2$ choices, tribe $D$ has $4$, exactly as above), we test the combined-territory observation — soon to be named Hall's condition — across all $2^6$ subsets of tribes, then confirm a valid totem assignment (a perfect matching) really exists:

```python
<!-- include: code/dm/12 - Matchings in graphs/02_python.py -->
```
...
![Figure 33: The good graph is also good from right to left](12 - Matchings in graphs_images/img-5.jpeg)

**Theorem 12.2 (The Marriage Theorem).** A bipartite graph has a perfect matching if and only if  $|A| = |B|$  and for any subset of (say)  $k$  nodes of  $A$  there are at least  $k$  nodes in  $B$  that are connected to one of them.
...
![Figure 34: Goodness lost when two nodes are removed](12 - Matchings in graphs_images/img-6.jpeg)
...
Before we do, it is worth seeing the *existence* question handed to a solver rather than enumerated by hand. Instead of walking the $150!$ pairings, we give **z3** (an SMT solver) one boolean variable per edge meaning "this pair is matched," with an "exactly one" constraint at every node. Z3 returns SAT (with a witness matching) for a "good" graph, and UNSAT — a machine-checked proof of impossibility — for a graph violating Hall's condition (exercise 12.2(b): two left nodes whose only neighbor is the same right node):

```python
<!-- include: code/dm/12 - Matchings in graphs/03_python.py -->
```

Running it prints a real matching `{0: 0, 1: 1, 2: 2}` for the good graph and `None` for the Hall-violating one, confirming the Marriage Theorem operationally: a perfect matching exists exactly when Hall's condition holds.
...
> **12.6** Prove that if $G$ has a perfect matching, then $M$ matches up at least half of the nodes.

![Figure 35: An augmenting path in a bipartite graph](12 - Matchings in graphs_images/img-7.jpeg)
...
![Figure 36: Reaching nodes by almost augmenting paths. Only edges on these paths, and of  $M$ , are shown.](12 - Matchings in graphs_images/img-8.jpeg)
...
- If  $r$  is matched with a node  $q \in A$ , then we can append the edges  $sr$  and  $rq$  to  $Q$ , to get an almost augmenting path from  $u$  to  $q$ . So we can add  $q$  to  $S$ .

So if we find an edge connecting a node in  $S$  to a node not in  $T$ , we can either increase the size of  $M$  or increase the set  $S$  (and leave  $M$  as it was). Sooner or later we must encounter a situation where either  $M$  is a perfect matching (and we are done), or  $M$  is not perfect but no edge connects  $S$  to any node outside  $T$ .
...
Since no clean condition is available, the honest move is to *search*. We encode the question for **z3** as constraints — a position $\mathrm{pos}(v) \in \{0, \dots, n-1\}$ per node, all distinct, with cyclically consecutive nodes forced to be joined by an edge — and let the solver decide. We test a $5$-cycle with a chord (Hamiltonian) and an unbalanced bipartite graph with $|A| = 3 \neq 2 = |B|$ (where alternation around any cycle forces $|A| = |B|$, so no Hamiltonian cycle can exist):

```python
<!-- include: code/dm/12 - Matchings in graphs/04_python.py -->
```

Running it prints `C5(+chord) Hamiltonian cycle: [0, 1, 2, 3, 4, 0]` and `unbalanced bipartite (|A|=3,|B|=2) Hamiltonian cycle: None`, confirming Z3 both finds a cycle and proves none exists when the two sides differ in size.

> **12.9** Decide whether or not the graphs in Figure 39 have a Hamiltonian cycle.

<!-- carousel -->
![Figure 37 (panel 1 of 4): The matching algorithm on a subgraph of the grid. Stage 1 — an initial matching $M$ (thick edges) built up greedily.](12 - Matchings in graphs_images/img-9.jpeg)
![Figure 37 (panel 2 of 4): Stage 2 — growing the set $S$ of nodes reachable by almost augmenting paths (gray nodes), with the candidate edge being explored shown dashed; the open circles are still unmatched.](12 - Matchings in graphs_images/img-10.jpeg)
![Figure 37 (panel 3 of 4): Stage 3 — an augmenting path (dashed) found connecting two unmatched nodes (open circles), about to enlarge the matching.](12 - Matchings in graphs_images/img-11.jpeg)
![Figure 37 (panel 4 of 4): Stage 4 — the enlarged matching $M$ after augmenting; gray nodes mark the set $S$ reached on almost augmenting paths.](12 - Matchings in graphs_images/img-12.jpeg)
<!-- endcarousel -->

![Figure 38: A graph for trying out the algorithm](12 - Matchings in graphs_images/img-13.jpeg)

<!-- carousel -->
![Figure 39 (left): Do these graphs have a Hamilton cycle? The dodecahedron graph.](12 - Matchings in graphs_images/img-14.jpeg)
![Figure 39 (right): Do these graphs have a Hamilton cycle? The Petersen graph.](12 - Matchings in graphs_images/img-15.jpeg)
<!-- endcarousel -->
