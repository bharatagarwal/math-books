...
Both of these counts are far too large to list out, but we can verify the closed forms exactly. Using `sympy`'s arbitrary-precision integers, we compare the book's "ordered product divided by $k!$" form against the built-in `binomial()` — the lottery count $\binom{90}{5} = \frac{90\cdot 89\cdot 88\cdot 87\cdot 86}{5!}$ and the bridge count $\binom{52}{13} = \frac{52\cdot 51\cdot\ldots\cdot 40}{13!}$:

```python
<!-- include: code/dm/02 - Let us count/02_python.py -->
```

Running it prints `C(90,5)  lottery tickets = 43949268` and `C(52,13) bridge hands = 635013559600`, reproducing both of the book's exact figures.
...
Each of the party's small counts is "build the actual objects, then count them," so we can confirm them without ever invoking a formula — just enumerate. Using Python's `itertools`, we list every ordering of the 6 guests with Alice's chair fixed (getting $720$), every 2-subset of the 7 people (getting $21$, Eve's "$42/2$"), and every way to split the 6 players into unordered pairs across indistinguishable boards (getting $15$, Bob's sense of "matching"). Each enumerated count is then asserted equal to the book's closed form ($6! = 720$, $\binom{7}{2} = 21$, $720/48 = 5\cdot 3 = 15$):

```python
<!-- include: code/dm/02 - Let us count/01_python.py -->
```

Running it prints `seatings of 6 guests (Alice fixed) = 720`, `handshakes among 7 people = 21`, and `chess matchings of 6 (Bob's sense) = 15`, confirming the enumerated objects match the party's three answers exactly.
...
> *(Source footnote: In mathematics, one can distinguish various levels of "infinity"; for example, one can distinguish between the cardinalities of $\mathbb{Z}$ and $\mathbb{R}$. This is the subject matter of set theory and does not concern us here.)*
...
The intersection of two sets is the set consisting of those elements that are elements of both sets. The intersection of two sets $A$ and $B$ is denoted by $A\cap B$. For example, we have $G\cap D=\{Alice\}$. Two sets whose intersection is the empty set (in other words, have no element in common) are called disjoint.
...
We read this figure as follows. We want to select a subset called $S$. We start from the circle on the top (called a node). The node contains a question: is $a_{1}$ an element of $S$? The two arrows going out of this node are labeled with the two possible answers to this question (Yes and No). We make a decision and follow the appropriate arrow (also called an edge) to the node at the other end. This node contains the next question: is $a_{2}$ an element of $S$? Follow the arrow corresponding to your answer to the next node, which contains the third (and in this case last) question you have to answer to determine the subset: is $a_{3}$ an element of $S$? Giving an answer and following the appropriate arrow we get to a node, which contains a listing of the elements of $S$.
...
![Figure 1: A decision tree for selecting a subset of  $\{a, b, c\}$ .](02 - Let us count_images/img-0.jpeg)
...
We can give another proof of theorem 2.1. Again, the answer will be made clear by asking a question about subsets. But now we don't want to select a subset; what we want is to enumerate subsets, which means that we want to label them with numbers  $0, 1, 2, \ldots$  so that we can speak, say, about subset No. 23 of the set. In other words, we want to arrange the subsets of the set in a list and then speak about the 23rd subset on the list.
...
> *(Source footnote: The fact that $2^{10}$ is so close to $10^{3}$ is used — or rather misused — in the name "kilobyte", which means $1024$ bytes, although it should mean $1000$ bytes, just like a "kilogram" means $1000$ grams. Similarly, "megabyte" means $2^{20}$ bytes, which is close to $1$ million bytes, but not exactly the same.)*
...
![Figure 2: A decision tree for ordering the elements of  $\{a, b, c\}$  (the six permutations).](02 - Let us count_images/img-1.jpeg)
...
Rather than trust this for one value of $n$, we can turn Theorem 2.4 into a property tested against real enumeration over many random $n$. Using `hypothesis`, for each $n$ we enumerate `permutations(range(n))` and assert the count equals $n!$; for random $(n,k)$ we enumerate the $k$-subsets and assert the count equals $\binom{n}{k}$, and that multiplying back by $k!$ recovers the falling product $n(n-1)\cdots$ — exactly the "count ordered selections, then divide by $k!$" reasoning behind the lottery and bridge counts. A third property re-derives the handshake count $\binom{n}{2} = n(n-1)/2$ for many $n$:

```python
<!-- include: code/dm/02 - Let us count/03_python.py -->
```

Running it prints `3 passed`, confirming that across all sampled $n$ (and $k$) the enumerated counts match the factorial and binomial formulas.
