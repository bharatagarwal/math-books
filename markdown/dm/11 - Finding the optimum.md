...
![Figure 27: Failure of the greedy method. Construction costs are proportional to the distance. The first figure shows a cheapest (shortest) cycle through all 4 towns; the second shows the cycle obtained by the optimistic (greedy) method.](11 - Finding the optimum_images/img-0.jpeg)
...
We can watch this failure happen on a concrete instance. Take the $4$ towns of Figure 27 with explicit construction costs $c(ij)$: the greedy rule keeps grabbing the cheapest edge that keeps every town at degree $\leq 2$ without closing a short cycle, and we compare the cycle it produces against the exact optimum found by brute force over all permutations. (The same edge set used as a *tree* is optimal — that is the point of the contrast.)

```python
<!-- include: code/dm/11 - Finding the optimum/02_python.py -->
```

Running it prints `Greedy is worse: 16 > 11  (factor 1.455)` for the tour while `MST cost = 6`, confirming Figure 27's lesson: the greedy rule is exact for the cheapest tree yet $45\%$ over optimum for the cheapest tour. The greedy edges cost $1+2+4+9 = 16$, whereas a smarter ordering does better.
...
![Figure 28: The greedy tree is optimal](11 - Finding the optimum_images/img-1.jpeg)
...
We can confirm the conclusion directly with `networkx`. The optimistic/greedy method (add the cheapest edge that closes no cycle) is exactly Kruskal's algorithm, and the capital-rooted growth of exercise 11.3 (always add the cheapest edge leaving the already-built town set) is exactly Prim's. The proof we just finished says both equal the true minimum over *all* $n^{n-2}$ trees, so let's run Kruskal, Prim, and a brute-force minimum over every spanning tree on one weighted network and check the three costs coincide:

```python
<!-- include: code/dm/11 - Finding the optimum/01_python.py -->
```

Running it prints `All three agree on the minimum cost = 16`, confirming that the optimistic greedy method, the capital-rooted method, and the exhaustive minimum over all trees produce the same total cost — even when Kruskal and Prim happen to choose different edge sets along the way.
...
But we want to show at least one simple algorithm that, even though it does not give the best solution, never loses more than a factor of 2. We describe this algorithm in the case when the cost of an edge is just its length, but it would not make any difference to consider any other measure (like time, or the price of a ticket), at least as long as the costs $c(ij)$ satisfy the triangle inequality:
...
Both links in this chain are properties we can hand to `hypothesis` and let it attack on hundreds of random instances. The argument rests on two facts: $\text{cost(cheapest tree)} < \text{cost(cheapest tour)}$, and $\text{cost(constructed tour)} \leq 2\cdot\text{cost(cheapest tree)}$ — the latter holding because we walk every tree edge twice and the triangle inequality $c(ij)+c(jk)\geq c(ik)$ guarantees each shortcut only shortens the walk. We encode these as properties over random Euclidean town-sets (Euclidean distance satisfies the triangle inequality automatically):

```python
<!-- include: code/dm/11 - Finding the optimum/03_python.py -->
```

Running it prints `2 passed`, confirming over $600$ generated town-sets that every triple satisfies $c(ij)+c(jk)\geq c(ik)$ and that the double-tree shortcut tour never exceeds twice the optimum.
