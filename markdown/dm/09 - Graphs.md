...
Coming back to our problem, we see that we can represent the party by a graph very conveniently. Our concern is the number of people known by a given person. We can read this off the graph by counting the number of edges leaving a given node. This number is called the *degree* (or *valency*) of the node. The degree of node $v$ is denoted by $d(v)$. So $A$ has degree 4, $B$ has degree 2, etc.

![Figure 14: The graph depicting acquaintance between our friends](09 - Graphs_images/img-0.jpeg)

We can build this exact graph and read its degrees off with **networkx**. Encoding the acquaintances precisely as the text states them — Alice knows everybody else, Bob and Carl know each other, and Carl also knows Eve — reproduces the book's numbers and lets us count the three even-degree people directly:

```python
<!-- include: code/dm/09 - Graphs/01_python.py -->
```

Running it prints `degrees: {'Alice': 4, 'Bob': 2, 'Carl': 3, 'Diane': 1, 'Eve': 2}` and `sum(deg) = 12, 2*|E| = 12, |E| = 6`, confirming the figure's degrees and — looking ahead to Theorem 9.2 — that the degree sum is exactly twice the six edges.

![Figure 15: Some graphs with an odd number of nodes, with nodes of even degree marked](09 - Graphs_images/img-1.jpeg)

If Frank now arrives, and he does not know anybody, then we add a new node that is not connected to any other node. So this new node has degree 0.
...
![Figure 16: Some graphs with an even number of nodes, with nodes of even degree marked](09 - Graphs_images/img-2.jpeg)

![Figure 17: Building up a graph one edge at a time](09 - Graphs_images/img-3.jpeg)
...
The theorem claims something about *every* graph, so a single drawing is weak evidence. We can stress-test it with **hypothesis**, which generates hundreds of random graphs via `nx.gnp_random_graph(n, p)` (plus a second test that builds graphs from arbitrary edge lists, mirroring the "one edge at a time" induction above) and asserts on each that the count of odd-degree nodes is even — and, as a bonus, that $\sum_v d(v) = 2|E|$:

```python
<!-- include: code/dm/09 - Graphs/02_python.py -->
```

Running it prints `2 passed`, meaning the handshake lemma and the degree-sum identity survived every randomly generated graph — strong empirical backing for Theorem 9.1.
...
![Figure 18: Two paths and two cycles](09 - Graphs_images/img-4.jpeg)
...
This second proof hinges on a single arithmetic fact — "the sum of an odd number of odd numbers is odd" — so an even degree sum forces an *even* count of odd-degree nodes. We can let the SMT solver **z3** try to break it. Part 1 asks for $k$ odd integers (for $k=1,3,5,7,9$) whose sum is even; Part 2 models a small graph's degrees, asserts Theorem 9.2 ($\sum_v d(v)$ even), and asks whether the number of odd-degree nodes could be odd:

```python
<!-- include: code/dm/09 - Graphs/04_python.py -->
```
...
![Figure 19: A path in a graph connecting two nodes](09 - Graphs_images/img-5.jpeg)
...
<!-- carousel -->
![Figure 20 (left): The given paths $P$ from $a$ to $b$ and $Q$ from $b$ to $c$, which may intersect each other.](09 - Graphs_images/img-6.jpeg)

![Figure 20 (middle): Following $P$ to its first common node $d$ with $Q$, then following $Q$ on to $c$.](09 - Graphs_images/img-7.jpeg)

![Figure 20 (right): The resulting path from $a$ to $c$. How to select a path from $a$ to $c$, given a path from $a$ to $b$ and a path from $b$ to $c$?](09 - Graphs_images/img-8.jpeg)
<!-- endcarousel -->
...
Let $G$ be a graph that is not necessarily connected. $G$ will have connected subgraphs; for example, the subgraph consisting of a single node (and no edge) is connected. A connected component $H$ is a maximal subgraph that is connected; in other words, $H$ is a connected component if it is connected but every other subgraph of $G$ that contains $H$ is disconnected. It is clear that every node of $G$ belongs to some connected component. It follows by exercise 9.14 that different connected components of $G$ have no node in common (else, their union would be a connected subgraph containing both of them). In other words, every node of $G$ is contained in a unique connected component.

Let's make these definitions concrete with **networkx**. We build $K_5$ (and check it has $\binom{5}{2}=10$ edges), the path $P_5$ (with $5-1=4$ edges and exactly two degree-1 endpoints), and the cycle $C_5$ (with $5$ edges, every degree $2$). We then verify connectivity *by the book's definition* — a path exists between every pair of nodes — and split a graph of two triangles plus an isolated node into its connected components, checking that no edge crosses between them (the assertion of exercise 9.16):

```python
<!-- include: code/dm/09 - Graphs/03_python.py -->
```

Running it prints `K_5: |E| = 10, C(5,2) = 10` and `disconnected graph -> 3 components: [[0, 1, 2], [3, 4, 5], [6]]`, confirming the edge counts of the special graphs and the partition into connected components.
